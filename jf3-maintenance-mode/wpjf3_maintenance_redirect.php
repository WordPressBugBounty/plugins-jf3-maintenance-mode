...
if( !class_exists("wpjf3_maintenance_redirect") ) {
	class wpjf3_maintenance_redirect {
		...
		function get_admin_options() {
			...
			if ( isset( $notoptions[ $this->admin_options_name ] ) ) {
				$wpjf3_mr_options = array(
					...
					'maintenance_html'    => "<html><head><title>" . esc_html__( "Under construction" ) . "</title></head><body><h1 style='margin-left:auto;margin-right: auto;max-width: 500px;border:1px solid #000;color: #000;background-color: #fff;padding: 10px;margin-top:200px'>" . esc_html( $this->maintenance_html_body ) . "</h1></body></html>",
					'static_page'         => '',
					'active_tab'	    => '#about',
					'hide_coffee'         => false,
					'uninstall'           => false
		...
		// (php) add new IP
		function add_new_ip() {
			if ( !current_user_can('manage_options') ) wp_die( __( "Oh no you don't!" ) );
			...
			if( $rs ){
				...
				esc_html_e( 'Unable to add IP because of a database error. Please reload the page.' );
			}
			die();
		}
		...
		function toggle_ip_status(){
			...
			if( $rs ){
				// $this->print_unrestricted_ips();
				echo 'SUCCESS' . '|' . esc_html( $ip_id ) . '|' . $ip_active;
			}else{
		...
		function delete_ip(){
			...
			if( $rs ){
				...
				esc_html_e( 'Unable to delete IP because of a database error. Please reload the page.' );
			}
			die();
		}
		...
		function add_new_ak() {
			...
			if( $rs ){
				...
				esc_html_e( "Unable to add Access Key because of a database error. Please reload the page." );
			}
			die();
		}
		...
		function toggle_ak_status(){
			...
			if( $rs ){
				// $this->print_access_keys();
				echo 'SUCCESS' . '|' . esc_html( $ak_id ) . '|' . $ak_active;
			}else{
				echo 'ERROR';
			}
			die();
		...
		function delete_ak(){
			...
			if( $rs ){
				...
				esc_html_e( 'Unable to delete Access Key because of a database error. Please reload the page.' );
			}
			die();
		}
		...
		function resend_ak(){
			...
			$ak        = $wpdb->get_row( $sql );
			if( $ak ){
				$subject    = sprintf( /* translators: %s = name of the website */ esc_html__( "Access Key Link for %s" ), get_bloginfo() );
				$full_msg   = sprintf( /* translators: %s = name of the website */ esc_html__( "The following link will provide you temporary access to %s:" ), get_bloginfo() ) . "\n\n"; 
				$full_msg  .= esc_html__( "Please note that you must have cookies enabled for this to work." ) . "\n\n";
				$full_msg  .= get_bloginfo('url') . '?wpjf3_mr_temp_access_key=' . esc_attr( $ak->access_key );
			...
			<table class="widefat fixed wpjf3-table" cellspacing="0">
				...
				<tbody>
					...
					if( $ips ){
						foreach( $ips as $ip ){
							?>
							<tr id="wpjf-ip-<?php echo esc_attr( $ip->id ); ?>" valign="middle"  class="<?php echo $ip_row_class; ?>">
								<td class="column-wpjf3-ip-name"><?php echo esc_html( $ip->name ); ?></td>
								<td class="column-wpjf3-ip-ip"><?php echo esc_html( $ip->ip_address ); ?></td>
								<td class="column-wpjf3-ip-active" id="wpjf3_mr_ip_status_<?php echo esc_attr( $ip->id ); ?>" ><?php echo ( $ip->active == 1 ) ? __( 'Yes' ) : __( 'No' ); ?></td>
								<td class="column-wpjf3-actions">
									<span class='edit' id="wpjf3_mr_ip_status_<?php echo esc_attr( $ip->id ); ?>_action">
										<?php if( $ip->active == 1 ){ ?>
											<a href="javascript:wpjf3_mr_toggle_ip( 0, <?php echo esc_attr( $ip->id ); ?> );"><?php esc_html_e( "Disable" ); ?></a> | 
										<?php } else { ?>
											<a href="javascript:wpjf3_mr_toggle_ip( 1, <?php echo esc_attr( $ip->id ); ?> );"><?php esc_html_e( "Enable" ); ?></a> | 
										<?php } ?>
									</span>
									<span class='delete'>
										<a class='submitdelete' href="javascript:wpjf3_mr_delete_ip( <?php echo esc_attr( $ip->id ); ?>, '<?php echo esc_attr( addslashes( $ip->ip_address ) ); ?>' );" ><?php esc_html_e( "Delete" ); ?></a>
									</span>
								</td>
							</tr>
							<?php
				...
				<tbody>
					...
					if( $codes ){
						foreach( $codes as $code ){
							?>
							<tr id="wpjf-ak-<?php echo esc_attr( $code->id ); ?>" valign="middle"  class="<?php echo $ak_row_class; ?>">
								<td class="column-wpjf3-ak-name"><?php echo esc_html( $code->name ); ?></td>
								<td class="column-wpjf3-ak-email"><a href="mailto:<?php echo esc_attr( $code->email ); ?>" title="email <?php echo esc_attr( $code->email ); ?>"><?php echo esc_html( $code->email ); ?></a></td>
								<td class="column-wpjf3-ak-key"><?php echo esc_html( $code->access_key ); ?></td>
								<td class="column-wpjf3-ak-active" id="wpjf3_mr_ak_status_<?php echo esc_attr( $code->id ); ?>" ><?php echo ( $code->active == 1 ) ? esc_html_e( 'Yes' ) : esc_html_e( 'No' ); ?></td>
								<td class="column-wpjf3-actions">
									<span class='edit' id="wpjf3_mr_ak_status_<?php echo esc_attr( $code->id ); ?>_action">
										<?php if( $code->active == 1 ){ ?>
											<a href="javascript:wpjf3_mr_toggle_ak( 0, <?php echo esc_attr( $code->id ); ?> );"><?php esc_html_e( "Disable" ); ?></a> | 
										<?php }else{ ?>
											<a href="javascript:wpjf3_mr_toggle_ak( 1, <?php echo esc_attr( $code->id ); ?> );"><?php esc_html_e( "Enable" ); ?></a> | 
										<?php } ?>
									</span>
									<span class='resend'>
										<a class='submitdelete' id='submit_resend_<?php echo esc_attr( $code->id ); ?>' href="javascript:wpjf3_mr_resend_ak( <?php echo esc_attr( $code->id ); ?>, '<?php echo esc_attr( addslashes( $code->name ) ); ?>', '<?php echo esc_attr( addslashes( $code->email ) ); ?>' );" ><?php esc_html_e( "Resend Code" ); ?></a> | 
									</span>
									<span class='copy'>
										<a class='submitdelete' id='submit_copy_<?php echo esc_attr( $code->id ); ?>' href="javascript:wpjf3_mr_copy_ak( <?php echo esc_attr( $code->id ); ?>, '<?php echo esc_attr( $code->access_key ); ?>' );" ><?php esc_html_e( "Copy Link" ); ?></a> | 
									</span>
									<span class='delete'>
										<a class='submitdelete' id='submit_delete_<?php echo esc_attr( $code->id ); ?>' href="javascript:wpjf3_mr_delete_ak( <?php echo esc_attr( $code->id ); ?>, '<?php echo esc_attr( addslashes( $code->name ) ); ?>' );" ><?php esc_html_e( "Delete" ); ?></a>
									</span>
								</td>
							</tr>
							<?php
		...
		function plugin_settings_link( $links ) { 
			$settings_link = '<a href="'.admin_url( 'options-general.php?page=JF3_Maint_Redirect' ).'">'.esc_html__( 'Settings' ).'</a>'; 
		...
		function wpjf3_site_health() {
			...
			$result = array(
				'label'       => sprintf( /* translators: %s = "Maintenance Redirect", the name of the plugin, */ esc_attr__( "%s is not Enabled" ), "Maintenance Redirect" ),
				'status'      => 'good',
				'badge'       => array(
					'label' => esc_html__( 'Visibility' ),
					'color' => 'blue',
				),
				'description' => sprintf(
					'<p>%s</p>',
					esc_html__( 'Maintenance Redirect is not enabled and your site is visible to visitors.' )
				),
				'actions'     => sprintf(
			...
			if ( $wpjf3_mr_options['enable_redirect'] == 'YES' ) {
				...
				if ( $wpjf3_mr_options['header_type'] == "200" or $wpjf3_mr_options['method'] == "redirect" ) {
					...
					$result['description'] = sprintf(
						'<p>%s</p>',
						esc_html__( 'Maintenance Redirect is enabled and your site is not visible to visitors.' )
					);
				} else {
					...
					$result['description'] = sprintf(
						'<p>%s</p>',
						esc_html__( 'Maintenance Redirect is enabled and your site is not visible to visitors. Your redirection type is set to 503, which could harm your Google ranking if left on for any length of time.' )
					);
				}
			...
			<div class="wrap">
				<form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>" onsubmit="return wpjf3_mr_validate_form();">
					...
					<div class="switch-field">
						...
						<label for="radio-yes"><?php esc_html_e( "On" ); ?></label>
						...
						<label for="radio-no"><?php esc_html_e( "Off" ); ?></label>
					</div>
					...
					<div id="wpjf3_main_options" <?php if( $wpjf3_mr_options['enable_redirect'] == "NO" ) echo 'style="display:none;"'; ?> class="tabs">
						...
						<div id="unrestricted-ip" class="wpjf3_mr_admin_section tab-content">
							...
							<p><?php esc_html_e( "NB: If the user you wish to whitelist accesses the site through a proxy you will need to add the proxy's IP address rather than the user's." ); ?></p> 
							<p><?php esc_html_e( "Your IP address is:" ); ?>&nbsp;<a id="wpjf3_set_ip"><?php echo esc_html( $this->get_user_ip() ); ?></a> - <?php esc_html_e( "Your Class C is:" ); ?>&nbsp;<a id="wpjf3_set_ip_c"><?php echo esc_html( $this->get_user_class_c() ); ?></a></p>
							<script>
								jQuery( "#wpjf3_set_ip" ).click( function(){ jQuery( "#wpjf3_mr_new_ip_ip" ).val( "<?php echo esc_js( $this->get_user_ip() ); ?>" ) } );
								jQuery( "#wpjf3_set_ip_c" ).click( function(){ jQuery( "#wpjf3_mr_new_ip_ip" ).val( "<?php echo esc_js( $this->get_user_class_c() ); ?>" ) } );
							</script>
						...
						<div id="message" class="wpjf3_mr_admin_section tab-content">	
							...
							<div id="wpjf3_method_redirect" class="wpjf3_method_input" style="<?php if( $wpjf3_mr_options['method'] != "redirect" ) echo "display:none;"; ?>" >
								...
								<p><input type="text" name="wpjf3_mr_static_page" value="<?php echo esc_attr( $wpjf3_mr_options['static_page'] ); ?>" id="wpjf3_mr_static_page" style="width:100%"></p>
							</div>
							
							<?php /* Output the HTML in the form */ ?>
							...
							<div class="about-contents">
								...
								<div class="about-right">
									<div class="coffee-panel">
										...
										<p><?php esc_html_e( "That said, I have been asked if it's possible to \"buy you a coffee\" by way of saying thanks for my time and effort in maintaining it." ); ?></p>
